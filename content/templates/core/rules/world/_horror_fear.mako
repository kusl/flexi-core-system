...
${h2('Horror and Fear')}
    ...
    <p>If a character passes the RESO test against the target number then nothing happens.  Until his next turn when, if he is still
    looking at, or under the effects of, the source of the Fear he must amke the same test.  Fear does not stop unless the source is
    gone.</p>
    
    <p>If the character fails the test then his MIND damage limit is reduced by one, with an additional point lost for every six increments
    under the target number.  Every succesive failed test reduces it by one again.  Once the characters MIND reaches zero he must start
    taking mind criticals.  These work in exactly the same way as that described in the Mind Damage description of the Combat section. The
    character takes Hysterias but also has a chance of gaining Post Traumatic Stress.</p>
